...
const lostAnimalsFiltersFormSchema = z.object({
  state: z.string().nullable().optional(),
  city: z.string().nullable().optional(),
})

type LostAnimalsFiltersFormData = z.infer<typeof lostAnimalsFiltersFormSchema>

export function LostAnimalsFilters() {
  ...
  const lostAnimalsFiltersForm = useForm<LostAnimalsFiltersFormData>({
    resolver: zodResolver(lostAnimalsFiltersFormSchema),
  })

  function onSubmit({ city, state }: LostAnimalsFiltersFormData) {
    setIsFiltered(true)
    router.push(
  ...
  function handleClearFilters() {
    setIsFiltered(false)
    router.push(`/lost-animals?_page=${pageIndex}&_per_page=12&state=&city=`)
  }

  return (
    <Form {...lostAnimalsFiltersForm}>
      <form
        onSubmit={lostAnimalsFiltersForm.handleSubmit(onSubmit)}
            ...
            <FormItem>
              <Select
                ...
                onValueChange={(value) => {
                  ...
                  lostAnimalsFiltersForm.setValue('city', null)
                }}
              >
                <FormControl>
